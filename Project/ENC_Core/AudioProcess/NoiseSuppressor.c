                    ...
                    float* prev_analysis_signal_spectrum, float* signal_spectrum)
{
    ...
    float gain = 0.5f * (1.0f + (float)tanh(2.0f * avg_prob_speech - 1.0f));
...
void NoiseSuppressor_init(NoiseSuppressor* var, int suppression_level) {
    ...
    ThreeBandFilterBank_init(&var->splitting_filter);     //默认3bands
...
void NoiseSuppressor_AggregateWienerFilters(NoiseSuppressor* var, float* filter) {
    ...
    // Copy the contents of the filter_ array into filter0
    memcpy(filter0, var->channels_0.wiener_filter.filter_, kFftSizeBy2Plus1 * sizeof(float));
...
void NoiseSuppressor_Analyze(NoiseSuppressor* var, FrameBuffer* audio) {
    ...
    memcpy(y_band0, audio->split_data_[0], sizeof(float) * (kNsFrameSize));
    ...
    for (int ch = 0; ch < num_channels_; ++ch) {
        ...
        ChannelState* ch_p = &var->channels_0;
        /*memcpy(&ch_p, &var->channels_0, sizeof(ChannelState));*/
        float extended_frame[kFftSize] = { 0 };
        FormExtendedFrame(y_band0, ch_p->analyze_analysis_memory, extended_frame);
        ApplyFilterBankWindow(extended_frame);

        // Compute the magnitude spectrum.
        float fft_out[kFftSize + 2] = {0};
        ...
        float* fft_ptr = fft_out;   //不需要释放这个指针，并未分配内存
        for (int i = 0; i < kFftSizeBy2Plus1; ++i) {
        ...
        float* noise_spectrum_ = ch_p->noise_estimator.noise_spectrum_;
        ComputeSnr(ch_p->wiener_filter.filter_, ch_p->prev_analysis_signal_spectrum, signal_spectrum, ch_p->noise_estimator.prev_noise_spectrum_,
            noise_spectrum_, prior_snr, post_snr);
...
//需要注意的是，这里的fft_out是一行real一行img，需要注意
//
...
void NoiseSuppressor_Process(NoiseSuppressor* var, FrameBuffer* audio) {
    FilterBankState filter_bank_states = var->filter_bank_states_heap_;
    float fft_out[kFftSize + 2];
    float signal_spectrum[kFftSizeBy2Plus1];
    float y_band0[kNsFrameSize];
    float upper_band_gains[num_channels_];
    float gain_adjustments[num_channels_];
    // Compute the suppression filters for all channels.
    for (int ch = 0; ch < num_channels_; ++ch) {
        // Form an extended frame and apply analysis filter bank windowing.
       // memcpy(y_band0, audio->split_data_[0], sizeof(float) * (kNsFrameSize));
        float* y_band0 = audio->split_data_[0];
        ...
        if (num_bands_ > 1) {
            ...
            upper_band_gains[ch] = ComputeUpperBandsGain(var->suppression_params_.minimum_attenuating_gain, var->channels_0.wiener_filter.filter_,
    ...
    if (num_channels_ == 1) {
        memcpy(filter_data, var->channels_0.wiener_filter.filter_, kFftSizeBy2Plus1 * sizeof(float));
    }
    ...
    for (int ch = 0; ch < num_channels_; ++ch) {
        ...
        ////这里对输出的extended_frame还做了个scaling
        //for (int i = 0;i < 256;++i) {
    ...
    for (int ch = 0; ch < num_channels_; ++ch) {
        ...
        gain_adjustments[ch] = ComputeOverallScalingFactor(&var->suppression_params_, var->num_analyzed_frames_,
    ...
    if (num_channels_ > 1) {
        for (int ch = 1; ch < num_channels_; ++ch) {
            gain_adjustment = min_local(gain_adjustment, gain_adjustments[ch]);
        }
    }
    ...
    for (int ch = 0; ch < num_channels_; ++ch) {
        for (int b = 0; b < num_bands_; ++b) {
            //memcpy(y_band, audio->split_data_[b], sizeof(float) * (kNsFrameSize));
            float* y_band = audio->split_data_[b];
            for (int j = 0; j < kNsFrameSize; j++) {
