...
void WienerFilter_Update(WienerFilter* var, SuppressionParams* suppression_params_, int32_t num_analyzed_frames,
    float* noise_spectrum, float* prev_noise_spectrum, float* parametric_noise_spectrum, float* signal_spectrum) 
{
    for (int i = 0; i < kFftSizeBy2Plus1; ++i) {
        // Previous estimate based on previous frame with gain filter.
        float prev_tsa = var->spectrum_prev_process_[i] / (prev_noise_spectrum[i] + 0.0001f) * var->filter_[i];
        // Current estimate.
        float current_tsa;
...
float ComputeOverallScalingFactor(SuppressionParams* suppression_params_, int32_t num_analyzed_frames, float prior_speech_probability, 
