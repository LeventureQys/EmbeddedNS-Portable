...
void SplittingFilter::ThreeBandsAnalysis(const ChannelBuffer<float>* data,
  ...
  for (size_t i = 0; i < three_band_filter_banks_.size(); ++i) {
    three_band_filter_banks_[i].Analysis(rtc::ArrayView<const float, ThreeBandFilterBank::kFullBandSize>(data->channels_view()[i].data(),
        ThreeBandFilterBank::kFullBandSize),
        rtc::ArrayView<const rtc::ArrayView<float>,ThreeBandFilterBank::kNumBands>(bands->bands_view(i).data(), ThreeBandFilterBank::kNumBands));
  }
...
void SplittingFilter::ThreeBandsSynthesis(const ChannelBuffer<float>* bands,
  ...
  for (size_t i = 0; i < data->num_channels(); ++i) {
    three_band_filter_banks_[i].Synthesis(
        rtc::ArrayView<const rtc::ArrayView<float>,ThreeBandFilterBank::kNumBands>(bands->bands_view(i).data(), ThreeBandFilterBank::kNumBands),
        rtc::ArrayView<float, ThreeBandFilterBank::kFullBandSize>(data->channels_view()[i].data(),ThreeBandFilterBank::kFullBandSize));
  }
}
