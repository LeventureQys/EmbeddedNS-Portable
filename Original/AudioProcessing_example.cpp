...
bool process_wav_audio(const std::string& input_filename, const std::string& output_filename) {
	...
	std::unique_ptr<webrtc::NoiseSuppressor> m_NoiseSuppressor
		= std::make_unique<webrtc::NoiseSuppressor>(config, header.sampleRate, header.numChannels);
...
#ifdef FLOATMODE
	std::vector<float> pcm_data(frame_size * num_channels);
#else
	...
	while (total_processed_samples < num_samples) {
...
#ifdef FLOATMODE 
		input_file.read(reinterpret_cast<char*>(pcm_data.data()), samples_to_read * num_channels * sizeof(float));
#endif
#ifndef FLOATMODE
...
#ifdef FLOATMODE 
		if (input_file.gcount() != samples_to_read * num_channels * sizeof(float)) {
			// 可能已经到达文件末尾，进行补零操作
			std::fill(pcm_data.begin() + input_file.gcount() / sizeof(float), pcm_data.end(), 0);
		}
#endif
#ifndef FLOATMODE
...
#ifdef FLOATMODE
		const float* data_ptr_inner = pcm_data.data();
		const float* const* data_ptr = &data_ptr_inner;
		audio_buffer.CopyFrom(data_ptr, StreamConfig(header.sampleRate, header.numChannels));
#endif
#ifndef FLOATMODE
		audio_buffer.CopyFrom(pcm_data.data(), StreamConfig(header.sampleRate, header.numChannels));
#endif
		//audio_buffer.CopyFrom(pcm_data.data(), StreamConfig(header.sampleRate, header.numChannels));
		if (audio_buffer.num_bands() > 1) {
		...
		//for (int i = 0;i < 3;++i) {
		//	for (int j = 0;j < 160;j++) {
		//		aout[i][j] = audio_buffer.split_bands(0)[i][j];
		//	}
		//}
		...
		//for (int i = 0;i < 3;++i) {
		//	for (int j = 0;j < 160;j++) {
		//		bout[i][j] = audio_buffer.split_bands(0)[i][j];
		//	}
		//}
#ifdef FLOATMODE
		std::vector<float> output_pcm_data(frame_size * num_channels);
#endif
#ifndef FLOATMODE
...
#ifdef FLOATMODE
		// 创建一个 float* const 类型的数组
		std::vector<float*> temp_pointers(num_channels);
		for (int i = 0; i < num_channels; ++i) {
			temp_pointers[i] = output_pcm_data.data() + i * frame_size;
		}
		float* const* channel_pointers = temp_pointers.data(); //这里把float** 转化为 float* const*

		audio_buffer.CopyTo(StreamConfig(header.sampleRate, header.numChannels), channel_pointers);
#endif
#ifndef FLOATMODE
		...
		//把数据先拷贝出来
		//float* output_pcm=  new float[audio_buffer.kSplitBandSize];
		//std::memcpy(output_pcm, audio_buffer.split_bands(0), audio_buffer.kSplitBandSize * sizeof(float));
		//output_file.write(reinterpret_cast<char*>(output_pcm), audio_buffer.kSplitBandSize * sizeof(float));
		// 写入输出文件
#ifdef FLOATMODE 
		output_file.write(reinterpret_cast<char*>(output_pcm_data.data()), samples_to_read * num_channels * sizeof(float));
#endif
#ifndef FLOATMODE
...
int main(int argc, char* argv[])
{
	...
	input_folder = "D:\\WorkShop\\Github\\enc_-webrtc\\Original";
