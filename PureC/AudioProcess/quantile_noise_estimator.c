...
{
    memset(var->quantile_, 0, sizeof(float) * (kFftSizeBy2Plus1));
...
void QuantileNoiseEstimator_Estimate(QuantileNoiseEstimator* var, float* signal_spectrum, float* noise_spectrum)
{
    ...
    LogApproximation_vect(signal_spectrum, log_spectrum, kFftSizeBy2Plus1);
    ...
    for (int s = 0, k = 0; s < kSimult; ++s, k += kFftSizeBy2Plus1)
    {
        ...
        for (int i = 0, j = k; i < kFftSizeBy2Plus1; ++i, ++j) {
            ...
            float temps = log_spectrum[i] - var->log_quantile_[j];
            float tmp = abs_local(temps);
    ...
    if (quantile_index_to_return >= 0) {
        ...
        ExpApproximation_vect(log_spectrum, var->quantile_, kFftSizeBy2Plus1);
    }
