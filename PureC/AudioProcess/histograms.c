...
void Histograms_Update(SignalModel* features_, Histograms* var) {
    ...
    if (features_->lrt < kHistogramSize * kBinSizeLrt && features_->lrt >= 0.f) {
        ++var->lrt_[(int)(kOneByBinSizeLrt * features_->lrt)];
    }
    ...
    if (features_->spectral_flatness < kHistogramSize * kBinSizeSpecFlat &&
        ...
        ++var->spectral_flatness_[(int)(features_->spectral_flatness * kOneByBinSizeSpecFlat)];
    }
    ...
    if (features_->spectral_diff < kHistogramSize * kBinSizeSpecDiff &&
        ...
        ++var->spectral_diff_[(int)(features_->spectral_diff * kOneByBinSizeSpecDiff)];
    }
