    ...
    float* conservative_noise_spectrum, float* signal_spectrum, float signal_spectral_sum, float signal_energy)
{
    ...
    //需要注意的是，这里原先是申请了一个引用，在C语言中只能改成这种形式

    SignalModel* model = &var->signal_model_estimator_.features_;
    PriorSignalModel* prior_model = &var->signal_model_estimator_.prior_model_;
    ...
    ExpApproximationSignFlip(model->avg_log_lrt, inv_lrt, kFftSizeBy2Plus1);
